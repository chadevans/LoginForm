    ...
    require([
        ...
        return declare('LoginForm.widget.LoginForm', [_WidgetBase, _Widget, _Templated], {
            ...
            _setupEvents: function () {
                ...
                on(this.submitButton, 'click', lang.hitch(this, function (e) {
                    ...
                    if (user && pass) {
                        ...
                        if (this.loginmf) {
                            var action = this.loginmf;
                            var widget = this;
                            var loginObj = null;

                            mx.data.create({
                                entity: this.loginEntity,
                                callback: function (obj) {
                                    loginObj = obj;
                                    loginObj.set("Username", user);
                                    loginObj.set("Password", pass);
                                    mx.data.action({
                                        params: {
                                            actionname: action,
                                            applyto: "selection",
                                        ...
                                        callback: function (value) {
                                            var localUser = loginObj.get("IsLocalUser");
                                            if (localUser) {
                                                widget._doLogin(user, pass);
                                            } else {
                                                widget._doLogin(user, "Mendix01");
                                            }
                                        },
                                        error: function () {
                                            console.error(widget.id + '.login: Error while calling microflow');
                                        }
                                    });
                                },
                                error: function (e) {
            ...
            _validateCustom: function () {
                logger.debug(this.id + '.validateCustom');

                if (typeof this._indicator !== 'undefined' && this._indicator) {
                    this._indicator.stop();
                }

                this.messageNode.innerHTML = "The username or password you entered is incorrect.";
                domStyle.set(this.messageNode, 'display', 'block');
            },
